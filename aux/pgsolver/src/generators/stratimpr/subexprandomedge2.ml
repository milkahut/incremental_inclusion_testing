...
let generator_game_func _ = 
	...
	let header = Info.get_title "Exponential Strategy Iteration Generator Random Edge" in
	  
	let strat = ref None in

	SimpleArgs.parsedef [(["-cl"], Int(fun i -> cyclelen := i),
					  ...
					  (fun s ->
						...
						pacedlen := !pacedlen * !n;
						cyclelen := !cyclelen * !n)
					  (header ^ "Usage: expstratrandomedge [n]\n" ^ "Generates the [n]-th game.\n\n");
...
Generators.register_generator generator_game_func "expstratrandomedge" "Exponential Random Edge Generator";;