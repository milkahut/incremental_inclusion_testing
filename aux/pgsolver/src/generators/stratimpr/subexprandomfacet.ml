(*
	...
	f(n) ~ 1/(2\sqrt(pi)) * exp(2 \sqrt(n) - 1/2) / (n^(1/4))

	http://www.research.att.com/~njas/sequences/?q=2%2C7%2C34%2C209%2C1546
*)
...
let generator_game_func _ =
	...
	SimpleArgs.parsedef speclist (fun n -> bit_count := int_of_string n) (header ^ "Usage: subexprandomfacet [n]\n" ^
...
Generators.register_generator generator_game_func "subexprandomfacet" "Subexponential Random Facet Lower Bound Generator";;