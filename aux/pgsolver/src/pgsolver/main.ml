...
(*	  let game = Parserhelper.parse_from_channel in_channel !perform_sanity_check in *)
	  let game = (
		match !solver with
			LocalSolver _ ->
				let (init, g) = Tcsautomataparser.parse_explicit_initpg in_channel in
				initnode := init;
  ...
  | _ -> (
	  let (solution,strategy) =
		match !solver with
			  NoSolver            -> if !parse_sol = "" then (
										...
										Tcsautomataparser.parse_explicit_parity_solution (open_in !parse_sol)
									 )
	  ...
	  if (!print_strategies) then (
		  ...
		  message 1 (fun _ -> "{" ^ String.concat "," (List.map string_of_int !win0) ^ "}\n");
		  message 1 (fun _ -> "with strategy\n  ");
		  message 1 (fun _ -> "[" ^ String.concat "," !str0 ^ "]\n\n");
		  message 1 (fun _ -> "Player 1 wins from nodes:\n  ");
		  message 1 (fun _ -> "{" ^ String.concat "," (List.map string_of_int !win1) ^ "}\n");
		  message 1 (fun _ -> "with strategy\n  ");
