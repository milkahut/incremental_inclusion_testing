...
let pg_set_pl pg i pl = let (pr, pl, delta, desc) = pg.(i) in pg.(i) <- (pr, pl, delta, desc);;
...
let parse_parity_game ch =
	Array.map (fun (a,b,c,d) -> (a,b,c,(if d = "" then None else Some d))) (Tcsautomataparser.parse_explicit_pg ch)
...
let transposed_graph_remove_nodes game graph nodes =
	...
	List.iter (fun v ->
		graph.(v) <- [-1];
		Array.iter (fun w -> affected := IntSet.add w !affected) (pg_get_tr game v)
	) nodes;
	IntSet.iter (fun v ->
...
let pg_with_graph_remove_nodes game graph nodes =
	let check_succ = ref IntSet.empty in
	let check_pred = ref IntSet.empty in
	List.iter (fun v ->
		let (_, _, tr, _) = game.(v) in
		Array.iter (fun w -> check_pred := IntSet.add w !check_pred) tr;
		game.(v) <- (-1, -1, [||], None);
		List.iter (fun w -> check_succ := IntSet.add w !check_succ) graph.(v);
		graph.(v) <- [];
	) nodes;
	IntSet.iter (fun v ->
    ...
    while !todo <> [] do
      ...
      then (visited.(v) <- true;
            ...
            List.iter (fun w -> let c = scc_index.(w) in
                                if c > -1
                                then (DynArray.set topology c (IntSet.add !next_index (DynArray.get topology c));
                                      is_root := false))
                       succs)
    done;
...
let sccs_compute_leafs roots topology =
	let leafs = ref IntSet.empty in
	let rec process r =
		if topology.(r) = []
		then leafs := IntSet.add r !leafs
...
let attr_closure_inplace' game strategy player region include_region tgraph deltafilter overwrite_strat =
  ...
  let inattr v = attr.(v) || ((not include_region) && IntSet.mem v region) in

  IntSet.iter (fun v -> if include_region then attr.(v) <- true;
...
let attr_closure_inplace game strategy player region =
	attr_closure_inplace' game strategy player (IntSetUtils.of_list region) true
...
let attractor_closure_inplace_sol_strat game transp deltafilter sol strat pl0 pl1 =
	let sol0 = attr_closure_inplace' game strat 0 pl0 true transp (fun v -> not (IntSet.mem v pl1)) true in
	let sol1 = attr_closure_inplace' game strat 1 pl1 true transp (fun v -> not (IntSet.mem v pl0)) true in
...
let pg_set_closed pg nodeset pl =
    IntSet.for_all (fun q -> let (_, pl', delta, _) = pg.(q) in
        if pl = pl'
        then Array.fold_left (fun r i -> r || IntSet.mem i nodeset) false delta
        else Array.fold_left (fun r i -> r && IntSet.mem i nodeset) true delta
    ) nodeset;;

let pg_set_dominion solver pg nodeset pl =
	if pg_set_closed pg nodeset pl then (
        let l = IntSet.elements nodeset in
...
module SymbolicParityGame = struct
	...
	let to_paritygame (ht, gr) =
		...
		Hashtbl.iter (fun _ ind ->
			...
			let succs = Plainset.elements (DynamicGraph.get_node_succ ind gr) in
