...
let anti_propagation_inplace pg =
	...
	let rec reach iterfunc v pr doneset = function [] -> false |
		u::r -> let l = ref r in
			...
			iterfunc (fun w ->
				...
				else if (getpr w < pr) && (not (IntSet.mem w !d)) then (
					d := IntSet.add w !d;
					l := w::!l
				)
			) u;
	...
	for v = 0 to n - 1 do
		if ((getpr v > 0) &&
		   (not ((reach (fun f u -> Array.iter f (pg_get_tr pg u)) v (getpr v) IntSet.empty [v]) ||
                         (reach (fun f u -> List.iter f gr.(u)) v (getpr v) IntSet.empty [v]))))
