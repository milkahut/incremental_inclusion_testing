	...
	for i = 0 to n - 1 do
		...
		if (pr >= 0) then (
			...
			Array.iter (fun r ->
				...
				try
					let q = IntMap.find !r !s in
					s := IntMap.add !r (q + 1) !s
				with
					Not_found -> s := IntMap.add !r 1 !s
			) tr;
...
type node_valuation = int * int Plainset.plainset * int
...
let format_node_valuation (a, b, c) =
    let t = Plainset.fold (fun el s -> s ^ (string_of_int el) ^", ") b "" in
...
let empty_descending_relevance_ordered_set game node_total_ordering =
	Plainset.empty (flip (relevance_total_ordering game node_total_ordering))
...
let node_valuation_ordering game node_total_ordering (u, p, e) (v, q, f) =
	...
	let path_ordering p q =
		let r = Plainset.sym_diff p q in
		if Plainset.cardinal r = 0 then 0
		else let w = Plainset.min_elt r in
		     if ((is_even w) && Plainset.mem w q) || ((is_odd w) && Plainset.mem w p) then -1 else 1
    in
...
let evaluate_strategy game node_total_ordering strat =
	...
	DynamicGraph.iter_by (fun v ->
		...
		if (w = -1) && (DynamicGraph.depth_find v (fun u -> rel_ord u v <= 0)
		                                          ...
		                                          graph) then (
			let graph' = DynamicGraph.sub_graph_by_node_closure v (fun u f -> Plainset.iter f (DynamicGraph.get_node_pred u graph)) graph in
			let del_other_edges x m =
				Plainset.iter (fun y ->
					if not (Plainset.mem y m) then DynamicGraph.del_edge x y graph'
			...
			DynamicGraph.iter_by (fun u ->
				if rel_ord u v > 0 then (
					if pg_get_pr game u mod 2 = 0 then (
						let m = build_graph_reach_set v (fun x -> Plainset.filter ((!=) u) (DynamicGraph.get_node_pred x graph')) in
						DynamicGraph.iter (fun q _ ->
							if not (Plainset.mem q m)
							then let (a, b, c) = valu.(q) in
								 valu.(q) <- (a, Plainset.add u b, c)
						) graph';
						del_other_edges u m;
						Plainset.iter (fun q -> del_other_edges q m) m
					)
					else (
						let m = build_graph_reach_set u (fun x -> Plainset.filter ((!=) v) (DynamicGraph.get_node_pred x graph')) in
						Plainset.iter (fun q ->
							let (a, b, c) = valu.(q) in
							valu.(q) <- (a, Plainset.add u b, c)
						) m;
            ...
            then max_dist_loop_closure v (fun x -> Plainset.elements (DynamicGraph.get_node_pred x graph'))
                                         (fun x -> Plainset.elements (DynamicGraph.get_node_succ x graph')) f
            else min_dist_closure v (fun x -> Plainset.elements (DynamicGraph.get_node_pred x graph')) (fun _ -> f) ();
	    	...
	    	Some d -> (
			    ...
			    while !go do
			    	...
			    	match (lookup_node ("k" ^ string_of_int !i)) with
			    		Some e -> (
			    			l := (if Plainset.mem e vald then 1 else 0)::!l
			    		)
			    	|	None -> go := false
			    done
			   )
    ...
    while !go do
    	...
    	match (lookup_node ("e" ^ string_of_int !i), lookup_node ("h" ^ string_of_int !i)) with
    		(Some e, Some h) -> (
	    		...
	    		l' := (if Plainset.mem h vale then 1 else 0)::!l'
    		)
    	|	_ -> go := false;
    done;
...
(*            msg_tagged_nl 3 (fun _ -> "\nMade valuation:\n" ^ game_to_string g ^ "\n"); *)
    ...
    while (improvable !strat !valu) do
        ...
        if !enable_exp_bit_count then (
			...
			msg_tagged_nl 3 (fun _ -> "Iteration: " ^ s ^ "\r");
...
let improvement_policy_optimize_all_globally game' node_total_ordering old_strategy valu =
	...
	let check = ref (Plainset.empty compare) in
	for v = 0 to n - 1 do
		...
		if pg_get_pl game v = 1 then (
			...
			check := Plainset.add v !check;
		)
		else if (Plainset.cardinal edges > 1) then DynamicGraph.del_node v graph'
		else check := Plainset.add v !check
	done;
	while (not (Plainset.is_empty !check)) do
		let v = Plainset.min_elt !check in
		check := Plainset.remove v !check;
		if Plainset.cardinal (DynamicGraph.get_node_succ v graph') = 0 then (
			check := Plainset.union !check (DynamicGraph.get_node_pred v graph');
			DynamicGraph.del_node v graph'
		)
	done;
	DynamicGraph.iter (fun v _ -> valued.(v) <- true) graph';

	let todo = ref (Plainset.empty compare) in
	let rest = ref (Plainset.empty compare) in

	for v = 0 to n - 1 do
		if valued.(v)
		then todo := Plainset.union !todo (DynamicGraph.get_node_pred v graph)
		else rest := Plainset.add v !rest
	done;

	let compute_update' v u = let (a, b, c) = valu2.(u) in (a, Plainset.add v b, c + 1) in
	let compute_update v =
		let p = Plainset.filter (fun u -> valued.(u)) (DynamicGraph.get_node_succ v graph) in
		let pl = pg_get_pl game v in
		let cmp x y = if pl = 0 then valu_ord valu2.(x) valu2.(y) else valu_ord valu2.(y) valu2.(x) in
		Plainset.fold (fun u mx -> if cmp u mx > 0 then u else mx) p (Plainset.min_elt p)
	in

	while (not (Plainset.is_empty !rest)) do
		while (not (Plainset.is_empty !todo)) do
			let v = Plainset.min_elt !todo in
			todo := Plainset.remove v !todo;
			let succs = DynamicGraph.get_node_succ v graph in
			if not valued.(v) then (
				if Plainset.for_all (fun u -> valued.(u)) succs then (
					valued.(v) <- true;
					...
					if pg_get_pl game v = 0 then strategy.(v) <- u
				        ...
				        (Plainset.exists (fun u -> valued.(u) &&
				                                   (valu_ord (compute_update' v u) valu2.(v) = 0)) succs)
				then (valued.(v) <- true; failwith "impossible");
				if valued.(v) then (
					todo := Plainset.union !todo (DynamicGraph.get_node_pred v graph);
					rest := Plainset.remove v !rest
				)
			)
		done;
		if not (Plainset.is_empty !rest) then (
            let temp = Plainset.filter (fun v -> (pg_get_pl game v = 1) && (Plainset.exists (fun u -> valued.(u)) (DynamicGraph.get_node_succ v graph))) !rest in
            if Plainset.is_empty temp
            then failwith "crap"
            else let l = Plainset.fold (fun v l -> let u = compute_update v in (v, compute_update' v u)::l) temp [] in
...
let rec improvement_policy_snare_based sub_policy snare_sel game node_total_ordering (snares, enforce) old_strategy valu =
	...
	|	None -> (
			...
			Array.iteri (fun i (_, pl, tr, _) ->
				if pl = 0 then (
					Array.iter (fun j ->
						if (cmp j old_strategy.(i) > 0) && (Plainset.mem i (valx j)) then (
							let sna = ref IntSet.empty in
							let todo = ref (IntSet.singleton j) in
							while not (IntSet.is_empty !todo) do
								let k = IntSet.min_elt !todo in
								todo := IntSet.remove k !todo;
								if (not (IntSet.mem k !sna)) && (Plainset.mem i (valx k)) then (
									sna := IntSet.add k !sna;
									let (_, pl, tr, _) = game.(k) in
									if pl = 0 then todo := IntSet.add old_strategy.(k) !todo
									else Array.iter (fun j -> todo := IntSet.add j !todo) tr
								)
							done;
							let esc = ref Int2Set.empty in
							let str = ref Int2Set.empty in
							IntSet.iter (fun k ->
								let (_, pl, tr, _) = game.(k) in
								if i = k then str := Int2Set.add (i, j) !str
								else if pl = 0 then str := Int2Set.add (k,old_strategy.(k)) !str
								else Array.iter (fun j -> if not (IntSet.mem j !sna) then esc := Int2Set.add (k,j) !esc) tr
							) !sna;
			...
			let appsnares = List.filter (fun (_,_,esc) ->
				Int2Set.for_all (fun (i,j) ->
					not (Plainset.equal (valx i) (Plainset.add i (valx j)))
				) esc
			) !snares in
...
let improvement_policy_learn_cycles sub_policy game node_total_ordering (cycles, u) old_strategy valu =
    ...
    Array.iteri (fun i scc ->
    	if (List.length scc > 1) && (topology.(i) = []) then (
    		...
    		if (pg_get_pr game (List.hd c) mod 2 = 0) && (not (Plainset.mem c !cycles)) then (
    			cycles := Plainset.add c !cycles;
    			let fmt k =
					match (pg_get_desc game k) with
		                None -> string_of_int k
		            |   Some t -> t
		        in    			
    			message 2 (fun _ -> "\nLearned cycle #" ^ string_of_int (Plainset.cardinal !cycles) ^ " : " ^ ListUtils.format fmt c ^ "\n")
    		)
    	)
    ) sccs;
    (strategy, (!cycles, u'))
...
let cycle_enforce_cycles_compare (node0, node1, edge0, edge1) (node0', node1', edge0', edge1') =
	let c0 = IntSet.compare node0 node0' in
	let c1 = IntSet.compare node1 node1' in
	let c2 = IntMap.compare compare edge0 edge0' in
	let c3 = IntMap.compare compare edge1 edge1' in
	if c0 != 0 then c0
...
let improvement_policy_cycle_enforce game node_total_ordering (cycles, idx) old_strategy valu =
	...
	let get_cycles strategy =
		let cycles = ref (Plainset.empty cycle_enforce_cycles_compare) in
		let combined_strategy =
			Array.init (Array.length valu) (fun i ->
				if pg_get_pl game i = 0
				then strategy.(i)
		...
		Array.iteri (fun i scc ->
			if (List.length scc > 1) && (topology.(i) = []) then (
				let node0 = ref IntSet.empty in
				let node1 = ref IntSet.empty in
				let edge0 = ref IntMap.empty in
				let edge1 = ref IntMap.empty in
				List.iter (fun v ->
					if pg_get_pl game v = 0 then (
						node0 := IntSet.add v !node0;
						edge0 := IntMap.add v combined_strategy.(v) !edge0;
					)
					else (
						node1 := IntSet.add v !node1;
						edge1 := IntMap.add v combined_strategy.(v) !edge1;
					)
				) scc;
				cycles := Plainset.add (!node0, !node1, !edge0, !edge1) !cycles;
			)
		) sccs;
		!cycles
	in
	...
	let cyc_value v (node0, node1, edge0, edge1) =
		...
		let m = ref (IntSet.cardinal node1) in
		let nodecur = ref v in
		while !m > 0 do
			while (pg_get_pl game !nodecur = 0) do
				valcur := Plainset.add !nodecur !valcur;
				nodecur := IntMap.find !nodecur edge0
			done;

			Array.iter (fun w ->
				if not (IntMap.find !nodecur edge1 = w) then (
					let (e, pth, f) = valu.(w) in
					let valw = (e, Plainset.union pth !valcur, f) in
					match !valworst with
			...
			valcur := Plainset.add !nodecur !valcur;
			nodecur := IntMap.find !nodecur edge1;
			decr m;
		done;
	...
	while (not !finished) && (!c <= n) do
		if (pg_get_pl game !i = 0) then (
			...
			Array.iter (fun w ->
				new_strategy.(!i) <- w;
				if Plainset.subset (get_cycles new_strategy) cycles
				then pots := (A w, valu.(w))::!pots;
				new_strategy.(!i) <- old_strategy.(!i);
			) (pg_get_tr game !i);
			Plainset.iter (fun ((node0, node1, edge0, edge1) as cyc) ->
				if (IntSet.mem !i node0) then (
					IntMap.iter (fun v w ->
						new_strategy.(v) <- w;
					) edge0;
					if Plainset.subset (get_cycles new_strategy) cycles
	...
	then (new_strategy, (cycles, !i))
		...
		(new_strategy, (Plainset.union cycles (get_cycles new_strategy), idx))
	)
...
let improvement_policy_smart game' node_total_ordering _ old_strategy valu cycles =
	...
	let cycle_array = Array.make (Plainset.cardinal cycles) [] in
	let cycle_ptr = Array.make n (Plainset.empty compare) in
	let ctr = ref 0 in
	Plainset.iter (fun c ->
		cycle_array.(!ctr) <- c;
		List.iter (fun i -> cycle_ptr.(i) <- Plainset.add !ctr cycle_ptr.(i)) c;
		incr ctr;
	) cycles;
	let remove_cycle c =
		List.iter (fun i -> cycle_ptr.(i) <- Plainset.remove c cycle_ptr.(i)) cycle_array.(c);
		cycle_array.(c) <- []
	in
	...
	let cycle_value cycle v =
		...
		let valus = ref [] in
		let path = ref (Plainset.empty compare) in
		while not (!cycle = []) do
			...
			if (pg_get_pl game u = 1) && (not valued.(u)) then (
				...
				valus := (a, Plainset.union !path b, c)::!valus
			);
			path := Plainset.add u !path
		done;
	...
	while (not (Plainset.is_empty !rest)) do
		...
		if not (Plainset.is_empty !rest) then (
            ...
            if Plainset.is_empty temp
            then failwith "crap"
            else let l = Plainset.fold (fun v l -> let u = compute_update v in (v, u, compute_update' v u)::l) temp [] in
...
let improvement_policy_smart game node_total_ordering todo old_strategy valu cycles =
	...
	let todo = Plainset.filter cycle_applies (if Plainset.is_empty todo then cycles else todo) in

	if Plainset.is_empty todo then (
		let strat = improvement_policy_optimize_all_locally game node_total_ordering old_strategy valu in
		(strat, todo)
	)
	else (
        let strategy = Array.copy old_strategy in
		Plainset.iter (fun cycle ->
   			let fmt k =
				match (pg_get_desc game k) with
	                None -> string_of_int k
	            |   Some t -> t
...
let improvement_policy_optimize_roundrobin_ordering_next game (ordering, backtransform) (v,w) =
	backtransform.(((Plainmap.find (v,w) ordering)+1) mod (Array.length backtransform))
...
let improvement_policy_optimize_roundrobin_ordering_build game cmp =
	...
	let a = Array.of_list !l in
	Array.sort cmp a;
	let m = ref (Plainmap.empty compare) in
	Array.iteri (fun i e ->
		m := Plainmap.add e i !m
	) a;
	(!m, a)
...
let improvement_policy_optimize_fair_sub_exp_tie_break game _ occ old_strategy valu l =
	...
	let leadsto i j =
		...
		Plainset.mem (OptionUtils.get_some j) path
	in
	...
	let occ_of i j =
		let i = OptionUtils.get_some i in
   		let tr = pg_get_tr game i in
		let k = ArrayUtils.find (fun u -> Some u = j) tr in
		occ.(i).(k)
	in
	
	for i = 0 to n - 1 do
		t := "|" ^ string_of_int (occ_of (find ("w" ^ string_of_int i)) (find ("X" ^ string_of_int i))) ^ 
				   string_of_int (occ_of (find ("v" ^ string_of_int i)) (find ("X" ^ string_of_int i))) ^ 
				   string_of_int (occ_of (find ("a" ^ string_of_int i)) (find ("E" ^ string_of_int i))) ^ 
		           string_of_int (occ_of (find ("b" ^ string_of_int i)) (find ("E" ^ string_of_int i))) ^ !t;
		let fndfst = ref 0 in
		...
		let fndsnd = ref 0 in
		if (leadsto (find ("v" ^ string_of_int i)) (find ("X" ^ string_of_int i))) &&
...
let improvement_policy_vorobyov2_init_edges game =
	let ed = ref (Plainset.empty compare) in
	Array.iteri (fun i (pr, pl, tr, _) ->
		if (pr >= 0) && (pl = 0) then Array.iter (fun j ->
			ed := Plainset.add (i,j) !ed
		) tr		
	) game;
	[(pg_copy game, !ed, Plainset.empty compare, 0)]
...
let improvement_policy_vorobyov2 game node_compare
	...
	|	(ga, ed, av, nu)::stack -> (
		if strategy_improvable ga node_compare strategy valu then (
			let strate = ref (Plainset.empty compare) in
			let ga' = Array.mapi (fun i (pr, pl, tr, de) ->
				if (pr >= 0) && (pl = 0) then strate := Plainset.add (i,strategy.(i)) !strate;
				let tr' = if (pr >= 0) && (pl = 0) then [|strategy.(i)|] else tr in
				(pr, pl, tr', de)
			) ga in
			let entry = (ga', !strate, ed, max (Plainset.cardinal !strate) (Plainset.cardinal ed / 2)) in
			...
			if Array.length impr_edges = 0 then iterate stack
				...
				(strategy', (ga, Plainset.add (i,j) ed, Plainset.remove (i,j) av, nu - 1)::stack)
			)
		)
		else iterate stack
	)
...
let strategy_improvement_cycle_enforce game =
	strategy_improvement game initial_strategy_by_best_reward node_total_ordering_by_position improvement_policy_cycle_enforce ((Plainset.empty cycle_enforce_cycles_compare), 0) false "STRIMPR_INTTWO";;
...
let strategy_improvement_smart_policy game =
	strategy_improvement game initial_strategy_by_best_reward node_total_ordering_by_position (improvement_policy_learn_cycles improvement_policy_smart) (Plainset.empty compare, Plainset.empty compare) true "STRIMPR_SMART";;

let strategy_improvement_justlearn_policy game =
	strategy_improvement game initial_strategy_by_best_reward node_total_ordering_by_position (improvement_policy_learn_cycles (fun a b c d e f -> (improvement_policy_optimize_all_locally a b d e, c))) (Plainset.empty compare, Plainset.empty compare) true "STRIMPR_JULE";;
...
let strategy_improvement_optimize_roundrobin_policy_lower_bound game =
	let cmp = lower_bound_construction_round_robin_compare game in
...
let subsolvermap = ref StringMap.empty;;

let register_sub_solver solver_func identifier abbreviation description =
	if StringMap.mem identifier !subsolvermap
	then failwith ("Solver `" ^ identifier ^ "' already registered!\n")
	else subsolvermap := StringMap.add identifier (solver_func, abbreviation, description) !subsolvermap;;

let mem_sub_solver identifier = StringMap.mem identifier !subsolvermap;;

let find_sub_solver identifier = StringMap.find identifier !subsolvermap;;

let enum_sub_solvers it = StringMap.iter (fun i (f, a, d) -> it f i a d) !subsolvermap;;

let fold_sub_solvers fo b = StringMap.fold (fun i (f, a, d) x -> fo f i a d x) !subsolvermap b;;
