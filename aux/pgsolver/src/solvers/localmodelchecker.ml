...
let partially_solve (game: partial_paritygame) =
	...
	let compatindex newidx oldidx b =
		if Int2Set.equal newidx oldidx then true
		else let ((pr, _) as e) = Int2Set.max_elt (Int2SetUtils.sym_diff newidx oldidx) in
		     if Int2Set.mem e newidx
    ...
    let updateIndex prio index =
		...
		while !stay do
			if Int2Set.is_empty !index then (
				stay := false;
				index := Int2Set.add (prio, 1) !index
			)
			else let (pr, nu) = Int2Set.min_elt !index in
			     if pr <= prio then (
					index := Int2Set.remove (pr, nu) !index;
					if pr = prio then (
						index := Int2Set.add (pr, nu + 1) !index;
						stay := false
					)
				 )
				 else (
...
*)
	...
	let winsOn from towhere =
		let (pr, _) = Int2Set.max_elt (Int2Set.diff towhere from) in
		prioWinner pr
	in

	let format_index index =
		let l = ref [] in
		Int2Set.iter (fun (p,o) ->
